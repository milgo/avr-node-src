...
void HDLC_qt::frameDecode(QByteArray buffer, quint16 bytes_to_send)
{
    ...
    fcs = HDLC_crc16::crc_update_data(buffer);
    /* Start flag */
...
bool HDLC_qt::hdlcFrameCRC_check(int frame_index)
{
    ...
    crc_received |= (receive_frame_buffer[frame_index-2] & 0xFF); // lsb

    quint16 crc_calculated = HDLC_crc16::crc_update_data(receive_frame_buffer.left(frame_index-2));
